...
- Deno Deploy 2 does not support Deno KV queues (`Deno.Kv.enqueue()` or `Deno.Kv.listenQueue()`). Admission atomically
  writes the V3 request, pending marker, and bounded reservation; the pending marker is the durable reconciliation source.
...
- Terminal events expedite a pending marker, while provider-log reads and settlement stay off the inference request path.
