...
Yunwu frequently returned more output tokens than direct Codex and `ai.ubq.fi`. Two explanations were considered:

1. Yunwu ignored the requested output limit; or
2. Yunwu served a different model behind the `gpt-5.6-sol` name.

## Documentation findings

The exported Yunwu documentation states that `/v1/chat/completions` accepts `max_tokens` and describes it as the maximum
generated token count. However, that section is written for Yunwu's GPTs-compatible route and does not document
...
Source: [Yunwu API documentation](https://yunwu.apifox.cn/)

## Controlled 32-token tests

All three Yunwu requests used the same deliberately long prompt:
...
Yunwu's proxy almost certainly strips or fails to forward output-limit fields on its Codex route:

1. the public-facing compatibility layer accepts each field;
2. direct Codex rejects `max_output_tokens` as unsupported;
3. Yunwu returns HTTP 200 instead of propagating that rejection; and
4. Yunwu generates an ordinary uncapped completion.
...
The longer Yunwu outputs are not evidence of a substituted model. In the control, direct OpenAI generated even more
output than Yunwu.
...
These observations strongly support the hypothesis that Yunwu forwards to real Codex capacity, probably through pooled
