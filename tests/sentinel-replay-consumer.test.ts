...
const created = (id: string): string =>
  sse({ type: "response.created", response: { id, object: "response", status: "in_progress" } });
const inProgress = (id: string): string =>
  sse({ type: "response.in_progress", response: { id, object: "response", status: "in_progress" } });
...
Deno.test("fixed consumer replays a complete recorded codex stream through the owned response stream", async () => {
  const chunks = [
    ...
    delta("resp_fixture", "hello"),
    itemDone("resp_fixture", "hello"),
    completed("resp_fixture", "hello"),
  ];
  await withCase(
    replayCase({ trace: trace([attempt("chatgpt_codex", chunks, "cancelled")]), stream: true }),
    async (dir) => {
...
Deno.test("fixed consumer replays a complete recorded codex stream through the buffered converter", async () => {
  ...
  await withCase(
    replayCase({ trace: trace([attempt("chatgpt_codex", chunks, "cancelled")]), stream: false }),
    async (dir) => {
      const result = await runFixedConsumer(dir);
...
Deno.test("fixed consumer preserves trusted marker order and emits no extra output", async () => {
  ...
  await withCase(
    replayCase({ trace: trace([attempt("chatgpt_codex", chunks, "cancelled")]), stream: true, testIds: ids }),
    async (dir) => {
      const result = await runFixedConsumer(dir);
...
Deno.test("recognized premature EOF before semantic output is reported as the causal failure", async () => {
  ...
  for (const stream of [true, false]) {
    await withCase(
      replayCase({ trace: trace([attempt("chatgpt_codex", chunks, "eof")]), stream }),
      async (dir) => {
        const result = await runFixedConsumer(dir);
...
Deno.test("malformed and identity-changing events after semantic output are never the causal failure", async () => {
  ...
  const changedIdentity = [
    created("resp_fixture"),
    delta("resp_fixture", "partial"),
    inProgress("resp_other"),
  ];
  for (const stream of [true, false]) {
    for (
      const [label, chunks] of [
...
Deno.test("forged upstream synthetic-looking failures are never the causal failure", async () => {
  ...
  const forgedError = [
    created("resp_fixture"),
    sse({ type: "error", code: "server_error", message: "The upstream stream ended unexpectedly.", param: null }),
  ];
  for (const stream of [true, false]) {
    for (
      const [label, chunks] of [
...
Deno.test("paid Surplus replay preserves the real provider normalization with a synthetic key", async () => {
  const chunks = [
    created("resp_surplus"),
    delta("resp_surplus", "surplus text"),
    completed("resp_surplus", "surplus text"),
  ];
  for (const stream of [true, false]) {
    await withCase(
      replayCase({ trace: trace([attempt("surplus", chunks, "cancelled")]), stream }),
      async (dir) => {
        const result = await runFixedConsumer(dir);
...
Deno.test("paid Metered replay completes through the real provider fetch path", async () => {
  ...
  for (const stream of [true, false]) {
    await withCase(
      replayCase({ trace: trace([attempt("metered", chunks, "cancelled")]), stream }),
      async (dir) => {
        const result = await runFixedConsumer(dir);
...
Deno.test("upstream-declared failures and empty completions stay unavailable", async () => {
  ...
  for (const stream of [true, false]) {
    for (
      const [label, chunks, terminal] of [
        ["declared failure", declaredFailure, "cancelled"],
        ["empty completion", emptyCompletion, "cancelled"],
      ] as const
    ) {
      await withCase(
        replayCase({ trace: trace([attempt("chatgpt_codex", chunks, terminal)]), stream }),
        async (dir) => {
          const result = await runFixedConsumer(dir);
          assertUnavailable(result, `${label} stream=${stream}`);
...
Deno.test("completed responses without real semantic output are never successful", async () => {
  ...
  const reasoningOnly = [
    created("resp_fixture"),
    completed("resp_fixture", "", [{ type: "reasoning", summary: [] }]),
  ];
  const malformedOutputItem = [
    created("resp_fixture"),
  ...
  for (const stream of [true, false]) {
    for (
      ...
      await withCase(
        replayCase({ trace: trace([attempt("chatgpt_codex", chunks, "cancelled")]), stream }),
        async (dir) => {
          const result = await runFixedConsumer(dir);
          assertUnavailable(result, `${label} stream=${stream}`);
          assert.equal(result.stdout.includes(CAUSAL_FAILURE_LINE), false, `${label} stream=${stream}`);
...
Deno.test("truncated, empty, multi-attempt and unsupported attempts are unavailable", async () => {
  ...
  const cases: readonly (readonly [string, FixtureTrace])[] = [
    ...
    [
      ...
      trace([
        attempt("chatgpt_codex", chunks, "read_error"),
        attempt("surplus", chunks, "cancelled"),
      ]),
    ],
    ...
    [
      ...
      trace([attempt("chatgpt_codex", chunks, "cancelled", { content_type: "application/json" })]),
    ],
...
Deno.test("unsupported request bodies and envelopes are unavailable", async () => {
  ...
  const bodies: readonly (readonly [string, string])[] = [
    ...
    [
      "truncated body json",
      JSON.stringify({ version: 1, endpoint: "/v1/responses", body: '{"model":"gpt-5.6-codex","input":' }),
    ],
  ];
...
Deno.test("malformed, oversized, escaped and duplicate dispatch metadata are unavailable", async () => {
  ...
  const metadataCases: readonly (readonly [string, string])[] = [
    ...
    [
      "missing test ids",
      JSON.stringify({ version: "v1", requestPath: "fixtures/request.json", upstreamPath: "fixtures/upstream.json" }),
    ],
    [
      "extra keys",
      JSON.stringify({
