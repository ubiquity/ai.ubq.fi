...
Deno.test("banked reset settings are independent per API key and survive unrelated edits", async () => {
  ...
  const update = async (body: unknown) => await handleAdminApiKeysUpdate(new Request(url, { method: "PATCH", body: JSON.stringify(body) }));
  ...
  const unauthorized = await handler(new Request(url, { method: "PATCH", body: JSON.stringify({ id: first.id, banked_resets_enabled: true }) }));
