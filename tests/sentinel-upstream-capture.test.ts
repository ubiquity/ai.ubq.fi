...
Deno.test("frozen trace relationship: status and content_type are null together", () => {
  assert.throws(
    () =>
      parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), status: null, content_type: "application/json" })),
    /content type/,
  );
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), status: 204, content_type: null })),
    /content type/,
  );
  // A pending pre-header attempt is exactly null/null with no chunks.
  const preHeader = parseSentinelUpstreamTrace(invalidTrace({
    provider: "surplus",
    status: null,
    content_type: null,
    chunks_base64: [],
...
Deno.test("frozen trace relationship: fetch_error and header-bearing terminals", () => {
  ...
  assert.throws(
    () =>
      parseSentinelUpstreamTrace(invalidTrace({
        provider: "metered",
        status: 502,
        content_type: "application/json",
        chunks_base64: [],
  ...
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), status: 99 })),
    /status is invalid/,
  );
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), status: 600 })),
    /status is invalid/,
  );
});
...
Deno.test("strict parser rejects provider enum, keys, version, canonical base64, and attempt bounds", () => {
  ...
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), content_type: "text/plain" })),
    /content type/,
  );
  assert.throws(() => parseSentinelUpstreamTrace({ ...invalidTrace(baseAttempt()), version: 2 }), /version/);
  assert.throws(
    () => parseSentinelUpstreamTrace({ ...invalidTrace(baseAttempt()), extra: true }),
    /keys are invalid/,
  );
  // Non-canonical padded base64: YR== decodes to the same byte as YQ== but
  // leaves non-zero trailing bits, so it must be rejected.
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), chunks_base64: ["YR=="] })),
    /base64/,
  );
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), chunks_base64: ["a"] })),
    /base64/,
  );
  assert.throws(
    () => parseSentinelUpstreamTrace(invalidTrace({ ...baseAttempt(), chunks_base64: ["YQ"] })),
    /base64/,
  );
...
Deno.test("recorder wrapper is lazy, forwards bytes, and records a complete EOF trace", async () => {
  ...
  assert.deepEqual(trace.attempts.map((attempt) => attempt.provider), ["surplus"]);
...
Deno.test("recorder preserves the original read error object and marks the trace read_error", async () => {
  ...
  const wrapped = recorder.startAttempt("metered").wrap(
  ...
  assert.equal(attempt.status, 200);
  assert.deepEqual(attempt.chunks_base64.map((chunk) => new TextDecoder().decode(base64UrlDecode(chunk))), [
    "partial-async",
  ]);
  recorder.dispose();
});
...
Deno.test("recorder forwards cancellation with the exact reason and surfaces reader cancel rejections", async () => {
  ...
  const wrapped = recorder.startAttempt("cerebras").wrap(
    new Response(blocking, { status: 200, headers: { "Content-Type": "text/event-stream" } }),
  );
  ...
  const wrappedReject = recorderReject.startAttempt("chatgpt_codex").wrap(
    new Response(rejecting, { status: 200, headers: { "Content-Type": "application/json" } }),
  );
  await assert.rejects(
    wrappedReject.body!.cancel(new DOMException("abort", "AbortError")),
    (error: unknown) => error === cancelFailure,
  );
...
Deno.test("recorder bounds: attempts, chunks, and bytes with permanent truncation flags", async () => {
  ...
  const bigWrapped = byteRecorder.startAttempt("chatgpt_codex").wrap(
    new Response(bigSource, { status: 200, headers: { "Content-Type": "application/json" } }),
  );
  ...
  const manyWrapped = chunkRecorder.startAttempt("metered").wrap(
    new Response(manyChunks, { status: 200, headers: { "Content-Type": "text/event-stream" } }),
  );
...
Deno.test("recorder bounds: simultaneous chunk and byte exhaustion discloses both limits", async () => {
  ...
  assert.equal(
    ...
    SENTINEL_UPSTREAM_MAX_BYTES + 1,
    "capture bounds must not truncate the original response",
  );
  const trace = recorder.snapshotAndSeal();
  const attempt = trace.attempts[0]!;
...
Deno.test("sealed snapshot is immutable and repeated snapshots return the same trace", () => {
  const recorder = createSentinelUpstreamRecorder();
  recorder.startAttempt("chatgpt_codex").wrap(
  ...
  const postSeal = recorder.startAttempt("surplus").wrap(
    new Response("later", { status: 200, headers: { "Content-Type": "application/json" } }),
  );
...
Deno.test("canonical upstream JSON is deterministic and matches the v2 frame encoding", async () => {
  ...
  const wrapped = recorder.startAttempt("chatgpt_codex").wrap(
    new Response(source, { status: 200, headers: { "Content-Type": "application/json" } }),
  );
  const reader = wrapped.body!.getReader();
  for (;;) {
  ...
  partial.startAttempt("chatgpt_codex").wrap(
    new Response("ab", { status: 200, headers: { "Content-Type": "application/json" } }),
  );
...
const exportRoundTrip = async (
  ...
  for (let index = 0; index < stored.manifest.chunk_count; index += 1) {
    const row = await kv.get<Uint8Array<ArrayBuffer>>([
      ...SENTINEL_REPLAY_CHUNK_PREFIX,
      stored.manifest.capture_id,
      index,
    ]);
...
const buildRecordedTrace = async (
  terminal: "eof" | "cancelled",
): Promise<ReturnType<typeof parseSentinelUpstreamTrace>> => {
  const recorder = createSentinelUpstreamRecorder();
  const source = new ReadableStream<Uint8Array>({
    pull(controller) {
  ...
  const wrapped = recorder.startAttempt("chatgpt_codex").wrap(
    new Response(source, { status: 200, headers: { "Content-Type": "text/event-stream" } }),
  );
  const reader = wrapped.body!.getReader();
  if (terminal === "eof") {
    for (;;) {
...
Deno.test({
  ...
  async fn() {
    ...
    const input = syntheticInput(
      encoder.encode('{"model":"gpt-5.6-sol","stream":false,"input":"same-request"}'),
      "v2-fingerprint-binding",
    );
    try {
      ...
      const storedComplete = await persistEncryptedSentinelReplay(
        { ...input, upstream: complete },
        failureObservation(),
        { kv, keyBytes, now, randomUuid: () => "capture-complete" },
      );
      const storedPartial = await persistEncryptedSentinelReplay(
        { ...input, upstream: partial },
        failureObservation(),
        { kv, keyBytes, now, randomUuid: () => "capture-partial" },
      );
      ...
      const completePlaintext = await decryptExportedSentinelReplay(
        await exportRoundTrip(storedComplete, kv),
        keyBytes,
      );
      assert.equal(completePlaintext.version, 2);
      assert.deepEqual(completePlaintext.upstream, complete);
      const partialPlaintext = await decryptExportedSentinelReplay(
        await exportRoundTrip(storedPartial, kv),
        keyBytes,
      );
...
Deno.test({
  ...
  async fn() {
    ...
    const now = () => 1_700_000_000_000;
    const input = syntheticInput(
      encoder.encode('{"model":"gpt-5.6-sol","stream":false,"input":"tamper"}'),
      "v2-tamper",
    );
    const encoderTrace = () => {
      const recorder = createSentinelUpstreamRecorder();
      recorder.startAttempt("chatgpt_codex").wrap(
        new Response("upstream-500", { status: 500, headers: { "Content-Type": "application/json" } }),
      );
      const trace = recorder.snapshotAndSeal();
      recorder.dispose();
      return trace;
...
Deno.test({
  ...
  async fn() {
    ...
    try {
      ...
      assert.equal(stored.status, "stored");
      const plaintext = await decryptExportedSentinelReplay(
        await exportRoundTrip(stored, kv),
        keyBytes,
      );
