...
Deno.test("openai: dynamic Surplus routing bypasses catalog backoff without a fake Codex attempt", async () => {
  const model = "glm-5.2";
  const keyId = "dynamic-surplus-catalog-recovery";
  ...
  try {
    // Reproduce the production failure mode: an earlier catalog request arms
    // the failure backoff before this authoritative inference request begins.
    const failedCatalog = await fetchSurplusModels({
      apiKey: "surplus-test-key",
      force: true,
      fetcher: () => Promise.resolve(new Response("catalog unavailable", { status: 503 })),
    });
    assert.equal(failedCatalog, null);
    ...
    const response = await withFetchMock(
      (url) => {
        ...
        if (url === "https://chatgpt.com/backend-api/codex/responses") {
          codexCalls += 1;
          return new Response(JSON.stringify({ error: { message: "Codex must not receive this model" } }), {
            status: 429,
            headers: { "Content-Type": "application/json" },
          });
        }
        throw new Error(`Unexpected upstream request in dynamic Surplus routing test: ${url}`);
      },
      () =>
        handleChatCompletions(
          new Request("https://ai.ubq.fi/v1/chat/completions", {
    ...
    const idKey = keyToString(["ubq_ai", "api_keys", "id", keyId]);
    const hashKey = keyToString(["ubq_ai", "api_keys", "hash", `hash-${keyId}`]);
    kvStore.set(idKey, {
      ...(kvStore.get(idKey) as Record<string, unknown>),
      paid_fallback_enabled: false,
    });
    kvStore.set(hashKey, {
      ...(kvStore.get(hashKey) as Record<string, unknown>),
      paid_fallback_enabled: false,
    });
    const rejectedRequestId = `${requestId}-disabled`;
    const rejected = await withFetchMock(
      (url) => {
        throw new Error(`Disabled dynamic routing must not contact an upstream: ${url}`);
      },
      () =>
        handleChatCompletions(
          new Request("https://ai.ubq.fi/v1/chat/completions", {
          ...
          { keyId, kernelRepo: null, kernelOrg: null, requestId: rejectedRequestId, startedAtMs: Date.now() },
        ),
    );
    assert.equal(rejected.status, 403);
    assert.equal(((await rejected.json()) as { error?: { code?: string } }).error?.code, "paid_fallback_disabled");
    assert.equal(getResponseTelemetry(rejected)?.provider, "gateway");
    assert.deepEqual(getResponseTelemetry(rejected)?.attemptedProviders, []);
  } finally {
    kvStore.delete(keyToString(["ubq_ai", "api_keys", "id", keyId]));
    kvStore.delete(keyToString(["ubq_ai", "api_keys", "hash", `hash-${keyId}`]));
    resetMeteredModelsCacheForTest();
