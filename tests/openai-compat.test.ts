...
Deno.test("openai: cache token usage reaches Chat clients and internal telemetry", async (t) => {
  ...
  await t.step(
    ...
    async () => {
      ...
      const independentlyReportedCacheRead = await withFetchMock(
        () =>
          sseResponse([
            `data: ${
              JSON.stringify({
        ...
        () =>
          handleResponses(
            new Request("https://ai.ubq.fi/v1/responses", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ model: DEFAULT_TEST_MODEL, input: "independently reported cache read" }),
            }),
          ),
      );
      assert.equal(getResponseTelemetry(independentlyReportedCacheRead)?.cachedInputTokens, 11);
      assert.equal(getResponseTelemetry(independentlyReportedCacheRead)?.usageTelemetryStatus, "reported");
