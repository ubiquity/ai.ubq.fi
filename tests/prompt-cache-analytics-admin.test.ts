...
Deno.test("admin cache analytics UI exposes bounded grouping, capped cohorts, and distinct retention", () => {
  ...
  assert.match(adminScript, /prompt_cache_key_present/);
  ...
  assert.match(adminScript, /chatgpt_codex:\s*"Codex"/);
  assert.match(adminScript, /surplus:\s*"Metered 1"/);
  assert.match(adminScript, /metered:\s*"Metered 2"/);
  assert.doesNotMatch(adminScript, /labels\.push\(group\.provider\)/);
