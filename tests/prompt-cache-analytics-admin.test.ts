...
Deno.test("admin cache analytics section is not rendered", () => {
  assert.doesNotMatch(adminScript, /data-prompt-cache-analytics/);
  assert.doesNotMatch(adminScript, /loadPromptCacheAnalytics/);
