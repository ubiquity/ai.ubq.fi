 ...
 * c1eab5fdbd6bde7d67f2da070116496558836884, `src/state.ts`. Mechanical changes:
 * Deno `.ts` specifiers, repository formatting, and non-consumed exports made
 * module-private. The fitting stages and the token estimator are unchanged.
 * See ./LICENSE and ./README.md.
 */
import type { CompactionState, FittedState, HistoryEntry, Message, ResolvedCompactOptions, ToolCall, ToolResult } from "./types.ts";
...
function mergeCallRuns(history: readonly HistoryEntry[], pinned: (e: HistoryEntry) => boolean): HistoryEntry[] {
  ...
  for (const entry of history) {
    ...
    const foldable = (e: HistoryEntry): boolean => !pinned(e) && e.text.length === 0 && typeof e.tool_calls?.[0] === "string";
...
function goalFromMessages(messages: readonly Message[]): string {
  return messages
    .filter((message) => message.role === "user" && message.text.trim().length > 0 && (message.toolResults ?? []).length === 0)
    .slice(-3)
...
export function fitState(
  ...
  const shrink = (index: number, change: (entry: HistoryEntry) => void): void => {
    const entry: HistoryEntry | undefined = history[index];
    if (!entry) return;
    change(entry);
    const now = entryTokens(entry);
    const previousTokens: number | undefined = perEntry[index];
    tokens += now - (previousTokens ?? 0);
    perEntry[index] = now;
  };
  ...
  const pinned = (entry: HistoryEntry): boolean => isPinned(entry.i, messages.length, options.preserveRecentMessages);
  const unpinnedFirst: number[] = [];
  const pinnedLast: number[] = [];
  for (const index of history.map((_, entryIndex) => entryIndex)) {
    const entry: HistoryEntry | undefined = history[index];
    if (!entry) continue;
    if (pinned(entry)) pinnedLast.push(index);
    else unpinnedFirst.push(index);
  }
  const order = [...unpinnedFirst, ...pinnedLast];
  ...
  for (const index of order) {
    ...
    const originalMessage: Message | undefined = messages[entry.i];
    const original = originalMessage?.text.length ?? entry.text.length;
    shrink(index, (e) => {
  ...
  for (const index of order) {
    ...
    left.add(index);
    const removedTokens: number | undefined = perEntry[index];
    tokens -= removedTokens ?? 0;
    if (fits()) {
      return fitted(
