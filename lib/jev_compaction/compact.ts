 ...
 * c1eab5fdbd6bde7d67f2da070116496558836884, `src/compact.ts`. Mechanical
 * changes: Deno `.ts` specifiers, repository formatting, non-consumed exports
 * made module-private, and the unused `reductionRatio` helper dropped. The
 * selection rules, batching, truncation semantics and decision mapping are
 * unchanged. See ./LICENSE and ./README.md.
 */
...
const DEFAULT_OPTIONS: ResolvedCompactOptions = {
  goal: "",
  keepThreshold: 0.5,
 ...
 */
function batchCalls(calls: readonly ToolCall[], stateTokens: number, options: Pick<ResolvedCompactOptions, "maxRequestTokens">): ToolCall[][] {
...
function decideCall(call: Pick<ToolCall, "id" | "tool" | "pinned">, answer: CallAnswer, options: Pick<ResolvedCompactOptions, "keepThreshold">): CallDecision {
...
async function askBatch(asker: JevAsker, state: CompactionState, batch: readonly ToolCall[]): Promise<Map<string, CallAnswer>> {
 ...
 */
function applyDecisions(messages: readonly Message[], decisions: readonly CallDecision[], calls: readonly ToolCall[], headChars: number): Message[] {
  ...
  for (const message of messages) {
    const touched =
      message.toolUses.some((tool) => actions.has(tool.tool_use_id)) || (message.toolResults ?? []).some((result) => actions.has(result.tool_use_id));
    if (!touched) {
      kept.push(message);
      continue;
    }
 ...
 */
export async function compact(messages: readonly Message[], asker: JevAsker, options: CompactOptions = {}): Promise<CompactResult> {
