...
const targetBindingFromInventory = (
  ...
  if (
    inventory.status !== "ready" || !inventoryFingerprint || !catalogVersionstamp || !authPoolVersionstamp || !model ||
...
const resolveBoundTarget = async (
  ...
  if (!currentBinding || !sameTargetCore(currentBinding, expected)) {
...
const selectCampaignTarget = async (
  ...
  for (const record of records) {
    const terminal = record.evidence && record.evidence.started_at_ms > 0 &&
      ...
      ["completed", "inconclusive", "failed"].includes(record.evidence.outcome);
