...
export type HarmonyTransportCallOptions = Readonly<{
  signal?: AbortSignal;
}>;

export type HarmonyTransport = (
  body: Record<string, unknown>,
  options?: HarmonyTransportCallOptions,
) => Promise<Response>;
...
export const createCerebrasTransport = (options: HarmonyTransportOptions = {}): HarmonyTransport => {
  return (body, callOptions) => {
    const signals = [options.signal, callOptions?.signal].filter((s): s is AbortSignal => s !== undefined);
    const signal = signals.length === 0 ? undefined : signals.length === 1 ? signals[0] : AbortSignal.any(signals);
    return fetchCerebrasChatCompletions(body, {
...
export const runHarmonyTurn = async (
  ...
  turnOptions?: HarmonyTransportCallOptions,
): Promise<RunTurnResult> => {
  const built = buildCerebrasHarmonyRequest(options);
  const response = await transport(built.body, turnOptions);
