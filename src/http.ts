...
const isCredentialedOrigin = (value: string): boolean => {
  try {
    const url = new URL(value);
    if (url.origin !== value) return false;
    if (url.origin === "https://ai.ubq.fi") return true;
    if (
      url.protocol === "https:" &&
      (url.hostname === "ai-ubq-fi.deno.dev" || url.hostname === "ai-ubq-fi.ubiquity-dao.deno.net" ||
        DENO_PREVIEW_HOST.test(url.hostname))
    ) return true;
