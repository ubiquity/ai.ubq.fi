...
export const loadDebugRoutingConfig = async (nowMs = Date.now()): Promise<DebugRoutingConfig> => {
  ...
  try {
    ...
    const entry = await kv.get(DEBUG_ROUTING_KEY, { consistency: "strong" });
...
export const setDebugRoutingConfig = async (scenario: DebugRoutingScenario, durationMs: number, nowMs = Date.now()): Promise<DebugRoutingConfig> => {
