...
/** Validates every codex reset usage setting row under its durable prefix. */
const auditCodexResetUsage = async (kv: Deno.Kv, errors: string[]): Promise<void> => {
  for await (const entry of kv.list({ prefix: CODEX_RESET_USAGE_PREFIX })) {
    ...
    const validValue = isRecord(entry.value) && typeof entry.value.enabled === "boolean";
    if (!validValue) {
