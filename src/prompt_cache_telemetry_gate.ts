...
type CounterPair = Readonly<{ completed: bigint; reported: bigint; invalid: bigint }>;
...
const counterKey = (
  ...
  route: PromptCacheTelemetryRoute,
  counter: "completed" | "reported" | "invalid",
): Deno.KvKey => [
...
const counterPair = (completedValue: unknown, reportedValue: unknown, invalidValue: unknown): CounterPair | null => {
  const completedMissing = completedValue === null;
  const reportedMissing = reportedValue === null;
  const invalidMissing = invalidValue === null;
  if (completedMissing && reportedMissing && invalidMissing) return { completed: 0n, reported: 0n, invalid: 0n };
  if (completedMissing || !isKvU64(completedValue)) return null;
  if (reportedMissing && invalidMissing) return { completed: completedValue.value, reported: 0n, invalid: 0n };
  if (reportedMissing) {
    if (!isKvU64(invalidValue) || invalidValue.value > completedValue.value) return null;
    return { completed: completedValue.value, reported: 0n, invalid: invalidValue.value };
  }
  if (invalidMissing) {
    if (!isKvU64(reportedValue) || reportedValue.value > completedValue.value) return null;
    return { completed: completedValue.value, reported: reportedValue.value, invalid: 0n };
  }
  if (!isKvU64(reportedValue) || reportedValue.value > completedValue.value) return null;
  if (!isKvU64(invalidValue) || invalidValue.value > completedValue.value) return null;
  if (reportedValue.value + invalidValue.value > completedValue.value) return null;
  return { completed: completedValue.value, reported: reportedValue.value, invalid: invalidValue.value };
};

const reportedCoverage = (counters: CounterPair): number | null =>
  counters.completed === 0n ? null : Number((counters.reported * 1_000_000n) / counters.completed) / 1_000_000;

const passesReportedCoverage = (counters: CounterPair): boolean =>
  counters.completed > 0n && counters.reported * COVERAGE_DENOMINATOR >= counters.completed * COVERAGE_NUMERATOR;
...
const counterSummary = (counters: CounterPair): PromptCacheTelemetryCounterSummary => ({
  ...
  reported_coverage: reportedCoverage(counters),
  reported_coverage_passed: passesReportedCoverage(counters),
});
...
export const readPromptCacheTelemetryBaseline = async (
  ...
  try {
    entriesByRoute = await Promise.all(
      PROMPT_CACHE_TELEMETRY_ROUTES.map(async (route) => {
        const [completed, reported, invalid] = await Promise.all([
        ...
        return { route, completed, reported, invalid };
      }),
    );
  } catch {
  ...
  for (const { route, completed, reported, invalid } of entriesByRoute) {
    const counters = counterPair(completed.value, reported.value, invalid.value);
    if (!counters) {
      return {
  ...
  const aggregate: CounterPair = countersByRoute.reduce<CounterPair>(
    ...
    { completed: 0n, reported: 0n, invalid: 0n },
  );
