...
const withoutLegacyTimeoutCircuit = (slot: CodexRoutingSlot): CodexRoutingSlot =>
  slot.upstream_timeout_blocked_until_ms === null && slot.probe_lease?.circuit !== "upstream_timeout" ? slot : {
    ...slot,
    upstream_timeout_blocked_until_ms: null,
    probe_lease: slot.probe_lease?.circuit === "upstream_timeout" ? null : slot.probe_lease,
  };
...
const selectCodexRoutingAccountsFromState = async (
  ...
  for (const auth of orderedAccounts) {
    ...
    if (slot.upstream_timeout_blocked_until_ms) {
      // A legacy timeout circuit uses the same fenced half-open lease as a
      // quota circuit. New inference requests never create this state.
