...
> => {
  ...
  if (input.policyCheck) schedulePaidFallbackBackfill(kv);
...
export const backfillPaidFallbackUsageRollups = async (
  ...
  for await (
    const entry of kv.list<PaidFallbackRequestV3>(selector, { consistency: "strong" })
  ) {
...
export const backfillPaidFallbackWindowTtls = async (
  ...
  for await (const entry of kv.list<PaidFallbackWindowV3>(selector, { consistency: "strong" })) {
    scanned += 1;
    ...
    if (rewritten >= limit && lastRewrittenKey) {
      ...
      for await (
        const _probe of kv.list<unknown>(
          { prefix: paidFallbackWindowV3GlobalPrefix, start: lastRewrittenKey },
          { consistency: "strong" },
        )
      ) {
        if (cursorRowSeen) {
  ...
  if (failed) {
    // Keep the cursor before a conflicted row so the next run retries its
    // missing TTL instead of reporting a complete sweep.
...
const paidFallbackBackfillNeedsRun = async (kv: Deno.Kv): Promise<boolean> => {
  ...
  // The completion marker is written only after both strong-consistency
  // sweeps finish. Live writes apply their own rollup marker and window TTL,
  // so a valid marker is the durable one-time guard and avoids scanning the
  // full namespaces on every later admission.
  return false;
};
...
const releasePaidFallbackBackfillLease = async (
  ...
  try {
    ...
    await kv.atomic().check(entry).delete(leaseKey).commit().catch(() => {});
  } catch {
 ...
 * Cursor keys let a first-request pass resume on a later request or revision,
...
export const runPaidFallbackBackfillV3 = async (
  ...
  try {
    ...
    if (complete) {
      const stateCommit = await kv.set(
      ...
      if (!stateCommit.ok) throw new Error("Paid fallback automatic backfill state changed concurrently.");
    }
    return {
...
const schedulePaidFallbackBackfill = (kvOverride?: Deno.Kv): void => {
  ...
  scheduledPaidFallbackBackfill = (kvOverride ? Promise.resolve(kvOverride) : resolveKv(undefined))
    .then(async (kv) => {
      if (!kv) return;
      await runPaidFallbackBackfillV3(kv);
    })
    .catch(() => {
      // A later paid-fallback admission retries after a transient bootstrap
      // or KV failure; the request path itself remains independent.
    })
    .finally(() => {
